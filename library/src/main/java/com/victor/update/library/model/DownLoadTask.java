...
public class DownLoadTask {
	...
	public DownLoadTask(Context context, UpdateData info, OnDownloadProgressListener listener) {
		mContext = context;
		updateData = info;
	...
	public void requestDownloadData() {
		...
		new UpdateTask().execute(updateData.getUrl());
	}
	...
	class UpdateTask extends AsyncTask<String, Integer, Bundle>{
		...
		protected Bundle doInBackground(String... params) {
			...
			try {
				if (HttpUtil.isNetEnable(mContext)){
					...
					File file = new File("/sdcard/" + updateData.getAppName() + ".apk");
					if (!file.exists()) {
						file.createNewFile();
					}
		...
		protected void onProgressUpdate(Integer... values) {
			...
			if (mOnDownloadProgressListener != null) {
				...
				if (progress < 100) {
					updateData.setProgress(values[0]);
					mOnDownloadProgressListener.onDownloadProgress(updateData);
				}
			}
		}
